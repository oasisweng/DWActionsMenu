...
class DWActionsMenu : UIView, DWResizeControlDelegate{
    ...
    init(target:UIViewController,actions:[DWAction],delegate:DWActionsMenuDelegate){
        super.init(frame: CGRectMake(0,
            target.view.frame.height-C.kMENUHEIGHT-C.kRESIZECONTROLHEIGHT,
            target.view.frame.width,
            C.kMENUHEIGHT+C.kRESIZECONTROLHEIGHT))
    ...
    private func setUp(){
        ...
        resizeControl!.frame = CGRectMake(0, 0, self.frame.size.width, C.kRESIZECONTROLHEIGHT)
        
        if (menuView == nil){
            //add menuview
            menuView = UIScrollView(frame: CGRectMake(0, C.kRESIZECONTROLHEIGHT, self.frame.width, C.kMENUHEIGHT))
        ...
        menuView!.frame = CGRectMake(0, C.kRESIZECONTROLHEIGHT, self.frame.width, C.kMENUHEIGHT)
        menuView!.contentSize = CGSizeMake(CGFloat(self.actions.count)*C.kMENUHEIGHT, C.kMENUHEIGHT)
    ...
    func prepare(){
        maxVisibleCols = Int(self.frame.width/C.kMENUHEIGHT)
    ...
    func resizeControlDidSlide(translation: CGPoint, resizeControl: DWResizeControl) {
        ...
        if (tMenuHeight>C.kMENUHEIGHT*CGFloat(self.maxRows!)){
            tMenuHeight = C.kMENUHEIGHT*CGFloat(self.maxRows!)
        } else if (tMenuHeight<C.kMENUHEIGHT) {
            tMenuHeight = C.kMENUHEIGHT
        }
        ...
        self.frame = CGRectMake(0, y-(tMenuHeight-self.menuView!.frame.height), self.frame.width, tMenuHeight+C.kRESIZECONTROLHEIGHT)
        self.resizeControl?.frame = CGRectMake(0, 0, self.frame.width, C.kRESIZECONTROLHEIGHT)
        self.menuView?.frame = CGRectMake(0, C.kRESIZECONTROLHEIGHT, self.frame.width, tMenuHeight)
        self.delegate?.actionsMenuDidResize?(self, translation: translation)
        
        var tRows = Int(tMenuHeight / C.kMENUHEIGHT)
        ...
        if (tRows != rows){
            ...
            menuView!.contentSize = CGSizeMake(CGFloat(cols)*C.kMENUHEIGHT, C.kMENUHEIGHT)
        }
    ...
    func getFrameForGridPath(gridPath:DWGridPath)->CGRect{
        ...
        var vOffset = (rowHeight-C.kBUTTONSIZE)/2
        var hOffset = (C.kMENUHEIGHT-C.kBUTTONSIZE)/2
        vOffset = vOffset>0 ? vOffset : 0
        hOffset = hOffset>0 ? hOffset : 0
        // button grid path
        var row = gridPath.row
        var col = gridPath.col
        var x = C.kMENUHEIGHT*CGFloat(col) + hOffset
        var y = C.kMENUHEIGHT*CGFloat(row) + vOffset
        
        return CGRectMake(x, y, C.kBUTTONSIZE, C.kBUTTONSIZE)
    }
